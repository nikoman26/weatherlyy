...
export const useWeatherStore = create<WeatherStore>((set, get) => ({
  ...
  // NEW: Fetch all airports and populate airportDetails map
  fetchAllAirports: async () => {
      ...
      try {
          ...
          set(state => ({
              airportDetails: { ...state.airportDetails, ...airportMap },
              isLoading: false
          }));
      } catch (error) {
          console.error('Bulk airport fetch failed:', error);
          set({ isLoading: false, error: 'Failed to load all airport data.' });
      }
  },

  // Simplified: Now relies on fetchAllAirports for bulk data
  fetchAirportsInBounds: async (bounds: { south: number, west: number, north: number, east: number }) => {
      // Since we are fetching all airports for the map, we can skip the bounds query for now.
      // If performance becomes an issue, this function can be updated to query the backend with bounds.
      console.log('Fetching all airports instead of filtering by bounds.');
